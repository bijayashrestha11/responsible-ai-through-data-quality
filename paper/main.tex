...
$r=0.40$ pooled (Spearman $0.46$). The relationship is strongest exactly where the thesis
expects---under MNAR, which is also the most harmful mechanism (Table~\ref{tab:regime}). As an
early-warning detector, thresholding $D_{\max}$ to flag $\Delta_{\mathrm{EO}}$ exceedance gives
AUROC $0.74$ (threshold $0.02$) and $0.78$ (threshold $0.05$). As expected, $D_{\max}$ predicts
...
importance-aware variant is marginally best---confirming that harm depends on which features
are disparate---but it requires the target at the gate, trading away the no-labels property; we
therefore keep $D_{\max}$ as the recommended default.
...
stage. Our gate computes $D_{\max}$ on an incoming batch and fails the pipeline when it exceeds
