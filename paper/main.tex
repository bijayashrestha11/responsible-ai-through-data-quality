...
the effect is swamped by equalized-odds estimation noise---subsampling Adult reproduces this null
with all else fixed, isolating sample size as the cause---so detectability requires sufficient
...
data, and while missing data is now an active fairness concern~\cite{martinezplumed2021}, it
remains under-addressed from an \emph{upstream, pipeline-detection} standpoint. This paper takes
that data-engineering stance: rather than measuring unfairness after a model exists, we ask what
...
\end{enumerate}
Our standing contribution is the predictive-validity evidence---a controlled, reproducible
benchmark relating a pre-training statistic to downstream fairness, with a derived threshold---that
audit-oriented work such as MDLA~\cite{mdla2026} identifies as needed but does not itself provide.
Equally central is where the signal \emph{fails}: across three datasets we find it holds on
Adult, reverses on COMPAS, and vanishes on German Credit, and we identify the two
properties---group structure and sample size---that govern which regime obtains. The honesty of
that boundary is a contribution, not a caveat.

\section{Related Work}\label{sec:related}
\textbf{Data quality as a fairness concern.}
A recurring theme is that fairness harms originate in data. This data-first stance has a lineage:
Sambasivan et al.~\cite{sambasivan2021} show upstream data problems compound into downstream
``data cascades,'' and a strand of data documentation---datasheets~\cite{gebru2021datasheets},
data cards~\cite{pushkarna2022datacards}---records a dataset's provenance and quality in prose. Our
contribution sits at the quantitative, automated end of that spectrum: a machine-checkable
disparity statistic enforced at a gate rather than prose documentation. Mart\'inez-Plumed et
al.~\cite{martinezplumed2021} argue missing values are a neglected dimension of fairness. For the
downstream side we adopt equalized odds~\cite{hardt2016} and name the impossibility
result~\cite{chouldechova2017} that constrains it when group base rates differ; a
survey~\cite{caton2024} gives the broader landscape.
...
The Missingness Demographic Leakage Audit~\cite{mdla2026} (preprint; still v1 as of writing) is a
manual, four-step retrospective \emph{audit} on ICU data testing whether missingness indicators
leak demographics; its findings are associative, and its response is post-hoc recalibration. It
shares our pre-deployment stance, so our differentiators are the two that do not turn on timing and
would hold even if such an audit were later automated: an \emph{automated pipeline validation gate},
and a \emph{calibrated predictive threshold} backed by benchmark evidence that a pre-training
statistic forecasts downstream fairness degradation---the prospective evaluation MDLA lists as
future work. We do not claim primacy of the idea; we claim the predictive-validity evidence.
...
vary three knobs: mechanism---missing completely at random, at random, or not at random
(MCAR / MAR / MNAR), the standard taxonomy of Rubin~\cite{rubin1976,littlerubin}---between-group
rate gap $\{0,0.05,0.10,0.20,0.30\}$, and overall rate $\{0.05,0.15,0.30\}$, with ten seeds, for
$450$ cells per dataset. Per cell we (1)~inject; (2)~compute $D_{\max}$ on the injected training data
...
We report equalized odds~\cite{hardt2016} as primary (the causal story---uneven corruption yields
uneven error rates---maps onto error-rate fairness) and name the impossibility
result~\cite{chouldechova2017}---calibration and error-rate balance cannot both hold when group
base rates differ---so the equalized-odds-specific framing is deliberate.
...
dissolving the linear signal. We confirm this is sample size, not a peculiarity of German, with a
controlled test: subsampling \emph{Adult} to $n=1000$---holding features, base rate, and domain
fixed---collapses its correlation to $r=0.007$, indistinguishable from German's null, and pooled
$r$ then rises monotonically with $n$ to $0.40$ at full size (Figure~\ref{fig:subsample}).
Detectability thus depends on sample size, a second axis orthogonal to the sign. Together the
three datasets map the boundary: the signal is present and sign-predictable-by-structure when the
sample is large enough (Adult, COMPAS) and undetectable when it is not (German).

\begin{figure}[t]\centering
\includegraphics[width=0.84\columnwidth]{fig6_subsample}
\caption{Detectability grows with sample size. Re-running the full grid on stratified subsamples
of Adult (all else fixed), pooled $r$ falls to German's null level at $n=1000$ and rises
monotonically to $0.40$ at full size.}
\label{fig:subsample}
\end{figure}

\textbf{Per-mechanism note.} On COMPAS the MAR signal is notably weaker ($r=-0.07$) than MCAR
($-0.52$) and MNAR ($-0.53$). A probe rules out the obvious cause---its zero-inflated MAR driver
(\texttt{priors\_count})---because Adult's MAR driver is in fact \emph{more} concentrated yet yields
a strong MAR signal ($r=+0.44$); we therefore report this per-mechanism variation as unexplained
rather than over-interpret it.
...
pipeline when it exceeds a policy threshold $\tau$; it requires only the features and the
protected-attribute label---no target, no model. We \emph{derive} $\tau$ from the benchmark
rather than assert it: on Adult, the F1-optimal cutoff for flagging a meaningful equalized-odds
increase is $D_{\max}\approx0.10$
...
with a moderate predictor. Practitioners should read the trade honestly: at this operating point a
precision of $0.26$ means roughly three in four flagged batches would not have produced a
meaningful equalized-odds increase, so the gate is a triage signal that routes data to human review
or closer auditing, not an automated block. The threshold is Adult-calibrated and, by the boundary
condition above, regime-dependent. On a batch with strong group-correlated missingness the gate fails
...
result) and its \emph{detectability} by sample size (the German null, reproduced by subsampling
Adult with all else fixed). This is itself the adjudication of the conflicting prior
results~\cite{bhatti2025,min2025}. We are careful not to
over-read the sign: the encoding-flip experiment identifies that the sign tracks \emph{which group}
carries the excess missingness, but the three structural properties that describe that group
(minority status, the sign of the base-rate difference, and the group--target correlation) co-vary
across our two datasets, so we do not claim any one of them as the identified cause. A concrete,
falsifiable next step would isolate it: a large-sample dataset in which these properties
\emph{dissociate}---for instance, one where the disadvantaged group is the \emph{majority} yet
carries the \emph{lower} base rate, so minority status and base-rate direction point opposite
ways---would break the confound and identify the causal property. We deliberately hold imputation
fixed---imputation choice is a known, separate fairness lever~\cite{feng2023}---and we leave
mitigation to future work: the gate \emph{detects}, it does not repair. Finally, our
